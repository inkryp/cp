...
int main() {
  ...
  freopen("o.txt", "w", stdout);
	int MAXN = 1e6 + 10;
  vector<long long> all(MAXN),two, three, four;
  for (long long i = 0; i < MAXN; ++i) {
    vector<pair<long long, int>> tmp = factorizer::Factorize(i + 1);
    all[i] = (int) tmp.size();
  }
  ...
  for (int i = 0; i < MAXN - 2; ++i) {
    if (all[i] == all[i + 1] && all[i + 1] == all[i + 2] && all[i] == 3) {
      three.push_back(i + 1);
    }
  }
  for (int i = 0; i < MAXN - 2; ++i) {
    if (all[i] == all[i + 1] && all[i + 1] == all[i + 2] && all[i + 2] == all[i + 3] && all[i] == 4) {
      four.push_back(i + 1);
    }
  }
  for (auto &x : two) {
